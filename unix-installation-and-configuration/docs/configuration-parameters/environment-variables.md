...
| ENABLE_CEF | This parameter is a Boolean value with a default value of 1. If set to 0, it disables the [Chromium Embedded Framework (CEF) https://en.wikipedia.org/wiki/Chromium_Embedded_Framework.](https://en.wikipedia.org/wiki/Chromium_Embedded_Framework) and at attempt to create an HTMLRenderer object will fail with an error meessage. See Note (below). |
...
| Variable | Notes |
| --- | ---  |
| EDITOR_COLUMNS_* | See Configuring the Editor on page 1. Can be one of EDITOR_COLUMNS_CHARACTER_ARRAY EDITOR_COLUMNS_CLASS EDITOR_COLUMNS_FUNCTION EDITOR_COLUMNS_NAMESPACE EDITOR_COLUMNS_NUMERIC_ARRAY |
