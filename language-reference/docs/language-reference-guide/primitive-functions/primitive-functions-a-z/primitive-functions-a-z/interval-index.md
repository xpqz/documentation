...
Like dyadic `⍳` (see [Index Of on page 1](../../non-scalar-selector-functions/index-of.md)), Interval Index works with major cells. For a vector these are its elements; for a matrix its rows, and so forth.


`X` and `Y` are compared using  the same logic as monadic `⍋` (see [Grade Up (Monadic) on page 1](../../non-scalar-selector-functions/grade-up-monadic.md)) which is independent of `⎕CT` and `⎕DCT`.
