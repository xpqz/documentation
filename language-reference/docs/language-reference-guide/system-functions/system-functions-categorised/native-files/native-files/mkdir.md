...
By default, for each file in `Y` the path must exist and the base name must not exist (see [File Name Parts](../../../system-functions-a-z/system-functions-a-z/nparts.md)), otherwise an error is signalled.
