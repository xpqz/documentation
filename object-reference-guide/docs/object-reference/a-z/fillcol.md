...
If [FStyle](FStyle.htm) is 0 (solid fill) FillCol defines the colour with which the object is filled. If [FStyle](FStyle.htm) is in the range 1-6 (pattern fill) it defines the colour of the lines that make up the pattern. The areas between the lines are filled using the colour specified by [BCol](BCol.htm), or are left undrawn (transparent) if [BCol](BCol.htm) is not specified. If [FStyle](FStyle.htm) contains the name of a [Bitmap](./bitmap.md) object, the value of FillCol is ignored.
